...
// saveAndRestoreFlavor saves the current flavor and returns a function to restore it
// This prevents global state contamination between tests
func saveAndRestoreFlavor(t *testing.T) func() {
...
func TestFetchOrCreateIPCCert_WithClusterCA(t *testing.T) {
	// Save and restore flavor to prevent global state contamination
	restoreFlavor := saveAndRestoreFlavor(t)
	defer restoreFlavor()
...
func TestBuildClusterClientTLSConfig_ValidationError(t *testing.T) {
	...
	config, err := buildClusterClientTLSConfig(caData)
	assert.Error(t, err)
	assert.Nil(t, config)
	...
	config2, err := buildClusterClientTLSConfig(caDataDisabled)
...
func TestFetchOrCreateIPCCert_ClusterAgentFlavor(t *testing.T) {
	...
	// Note: This test doesn't verify the specific SAN behavior for ClusterAgent
	// because that would require additional setup of the cluster agent endpoint configuration
	// The main goal is to ensure the ClusterAgent flavor code path is executed without errors
}
